...
class DummyModel extends AMLO\Model\AbstractAMLO
{
	...
	public function addPartyInRoleProxy($p1,$p2,$p3) { return $this->addPartyInRole($p1,$p2,$p3);}
}
...
class AbstractModelTest extends TestCase
{
	...
	public function testAddIdentifier()
	{
	    ...
	    $expected = $obj->getTurtleHeader() . "\n" .
            '<urn:resource:id> fibo-fnd-rel-rel:hasTag "id" ; ' .
	...
	{
	    $obj = DummyModel::fromArray(array());
	    $obj->addPartyInRoleProxy('urn:resource:subject', 'urn:resource:identity', 'fibo:role');
	    
	    $expected = $obj->getTurtleHeader() . "\n" .
    	    '<urn:resource:subject> fibo-fnd-pty-pty:hasPartyInRole ' .
    	    '[ a fibo:role ; fibo-fnd-rel-rel:hasIdentity <urn:resource:identity> ] .' ;
